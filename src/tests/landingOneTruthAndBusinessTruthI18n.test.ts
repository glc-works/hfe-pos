...
describe('Landing "One Transaction, One Truth" Story & Business Truth i18n Suite (Issue #33 Part B & A)', () => {
  it('contains complete 3-step event flow dictionary in Indonesian', () => {
    expect(idTranslations.landing.oneTruthTitle).toBe('Pesan Tanpa Antre')
    expect(idTranslations.landing.oneTruthStep1Title).toBe('1. Scan QR / Pesan Online')
    expect(idTranslations.landing.oneTruthStep2Title).toBe('2. Bayar Langsung')
    expect(idTranslations.landing.oneTruthStep3Title).toBe('3. Pesanan Siap')
  })

  it('contains complete 3-step event flow dictionary in English', () => {
    expect(enTranslations.landing.oneTruthTitle).toBe('Order Without the Wait')
    expect(enTranslations.landing.oneTruthStep1Title).toBe('1. Scan QR / Order Online')
    expect(enTranslations.landing.oneTruthStep2Title).toBe('2. Pay Instantly')
    expect(enTranslations.landing.oneTruthStep3Title).toBe('3. Order Ready')
  })
