...
const context = {
  ...
  governedAttempt: {
    ...
    transition: async (phase: GovernedCheckoutEvidence['phase'], evidence = {}) => {
...
function seedReviewedQuote(reviewed: any): void {
  durableEvidence = { phase: 'quote_ready', quote: {
    ...
    expires_at: reviewed.expiresAt, lines: reviewed.lines.map((line: any) => ({ ordinal: line.ordinal, item_id: line.itemId,
      quantity: line.quantity, modifier_ids: line.modifierIds, discount_allocated_minor: line.discountAllocatedMinor })),
