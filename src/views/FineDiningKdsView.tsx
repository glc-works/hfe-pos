...
import { Utensils, Flame, CheckCircle2, Clock, Sparkles } from 'lucide-react'
...
const INITIAL_FINE_DINING_ORDERS: TableCourseOrder[] = [
  {
    id: 'FD-101',
    ...
    courses: [
      ...
      { number: 3, name: 'Soup', dishes: ['Truffle Consommé with Wild Mushrooms'], status: 'Holding' },
      { number: 4, name: 'Sorbet Cleanser', dishes: ['Yuzu Hibiscus Granita'], status: 'Holding' },
      { number: 5, name: 'Main Course', dishes: ['A5 Miyazaki Wagyu Ribeye 150g'], status: 'Holding' },
      { number: 6, name: 'Dessert', dishes: ['Valrhona Dark Chocolate Sphere'], status: 'Holding' },
      { number: 7, name: 'Digestif', dishes: ['Matcha Infused Cognac & Mignardises'], status: 'Holding' },
    ]
  },
  {
    id: 'FD-102',
    tableNumber: 'Table 04',
    guestName: 'Ibu Ratna & Partner',
    menuType: '5-Course',
    currentCourseIndex: 2,
    courses: [
      { number: 1, name: 'Amuse-Bouche', dishes: ['Oyster Royale with Champagne Foam'], status: 'Served' },
      { number: 2, name: 'Appetizer', dishes: ['Smoked Duck Breast & Berry Reduction'], status: 'Served' },
      { number: 3, name: 'Soup', dishes: ['Lobster Bisque with Tarragon Butter'], status: 'Plating' },
      { number: 4, name: 'Main Course', dishes: ['Chilean Sea Bass with Saffron Sauce'], status: 'Holding' },
      { number: 5, name: 'Dessert', dishes: ['Grand Marnier Soufflé'], status: 'Holding' },
    ]
  }
]

export const FineDiningKdsView: React.FC = () => {
  const [orders, setOrders] = useState<TableCourseOrder[]>(INITIAL_FINE_DINING_ORDERS)
    ...
    <div className="flex-1 bg-slate-950 text-slate-100 p-4 space-y-4 overflow-y-auto">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-3 shadow-lg">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-base font-bold text-white">Executive Chef KDS Course Firing Matrix</h1>
            <p className="text-xs text-slate-400">Pengaturan Pacing hidangan Tasting Menu (3/5/7/9 Course) Realtime</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className="px-3 py-1 bg-slate-800 border border-slate-700 text-xs font-mono font-bold text-amber-400 rounded-xl">
            Live Kitchen Sync Active
          </span>
        </div>
      </div>

      {/* Course Firing Cards Grid */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {orders.map((order) => {
          const currentCourse = order.courses.find(c => c.status === 'Fired' || c.status === 'Plating') || order.courses[order.currentCourseIndex]
          ...
          return (
            <div key={order.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col justify-between space-y-4">
              ...
              {/* Course Matrix Timeline */}
              <div className="space-y-2">
                <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Matriks Firing Sequence</h3>
                <div className="space-y-2">
