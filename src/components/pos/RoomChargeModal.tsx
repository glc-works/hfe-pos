...
export const RoomChargeModal: React.FC<RoomChargeModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-slate-950/85 backdrop-blur-sm z-50 flex items-center justify-center p-3 sm:p-4 animate-fadeIn">
      <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-sm sm:max-w-md w-full p-4 sm:p-5 flex flex-col gap-3.5 shadow-2xl relative">
        ...
        <div className="flex items-center gap-3 border-b border-slate-800 pb-3">
          ...
          <div>
            <h3 className="text-base font-bold text-white">Charge to Hotel Room Folio</h3>
            <p className="text-[11px] text-slate-400">Posting Tagihan F&B ke Piutang Kamar Tamu (PMS)</p>
          </div>
        </div>
        ...
        {isSubmittedSuccess ? (
          <div className="py-8 flex flex-col items-center justify-center gap-2 text-center animate-scaleUp">
            ...
            <h4 className="text-sm font-bold text-white">Posting Folio Kamar Berhasil!</h4>
          ...
          <>
            ...
            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-400 font-semibold flex items-center justify-between">
                ...
                <span className="text-[10px] text-indigo-400">Contoh: 402, 305, 501</span>
              </label>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-950 border border-slate-800 text-white font-mono font-bold text-sm rounded-xl px-3 py-2 focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
            ...
            {matchedFolio ? (
              <div className={`p-3 rounded-2xl border flex flex-col gap-2 transition-all ${
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-bold text-xs text-white truncate">{matchedFolio.guestName}</span>
                  </div>
                  <span className={`text-[9px] font-black px-2 py-0.5 rounded-full font-mono ${
                    isCheckedIn ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-rose-500/20 text-rose-300'
                  }`}>
                ...
                <div className="grid grid-cols-2 gap-2 text-[11px] font-mono text-slate-300 pt-1 border-t border-slate-800">
                  ...
                  <div>
                    ...
                    <span className={isCreditSufficient ? 'text-emerald-400 font-bold' : 'text-rose-400 font-bold'}>
                ...
                {!isCreditSufficient && (
                  <div className="flex items-center gap-1 text-[10px] text-rose-300 bg-rose-500/20 p-1.5 rounded-lg font-semibold">
              ...
              <div className="p-3 bg-slate-950 border border-slate-800 rounded-2xl text-center text-slate-500 text-xs">
                Kamar tidak ditemukan di sistem PMS. Pastikan nomor kamar terdaftar.
              </div>
            )}

            {/* BILL & DOUBLE-ENTRY BREAKDOWN */}
            <div className="bg-slate-950 border border-slate-800 rounded-2xl p-2.5 flex flex-col gap-1.5 text-xs font-mono">
              ...
              <div className="flex items-center justify-between font-bold text-white text-xs pt-1 border-t border-slate-800">
                <span>Total Charge ke Kamar:</span>
                <span className="text-amber-400">{formatPrice(totalBill)}</span>
              </div>
            ...
            <div className="flex items-center gap-2">
              <div className="flex-1 flex flex-col gap-0.5">
                ...
                <input
                  ...
                  className="bg-slate-950 border border-slate-800 text-white font-mono text-center font-bold text-xs rounded-xl p-2 focus:outline-none focus:border-indigo-500"
                />
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full py-2.5 rounded-xl font-bold text-xs shadow-lg flex items-center justify-center gap-1.5 transition-all ${
                canSubmit
                  ? 'bg-indigo-500 hover:bg-indigo-400 text-white active:scale-98'
