...
export function FinancialStatusBanner({ status, notice, failureCode, onResume }: FinancialStatusBannerProps) {
  ...
  return (
    <div role="status" data-financial-failure-code={failureCode || undefined} className={`shrink-0 px-3 py-2 text-center text-xs font-bold ${tone}`}>
      <span>{t.cart.financialNotices[notice]}</span>
      {(notice === 'outcome_unknown' || notice === 'pending_core') && (
