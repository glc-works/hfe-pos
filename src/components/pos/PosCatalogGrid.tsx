...
export const PosCatalogGrid: React.FC<PosCatalogGridProps> = ({
  ...
  return (
    <div className="flex flex-col gap-2.5 select-none">
      ...
      <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-2xl p-2 sm:p-2.5 flex flex-col gap-2 shadow-md">
        ...
        <div className="relative flex items-center w-full">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2 bg-slate-100 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 focus:border-amber-500/60 rounded-xl text-xs sm:text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 outline-none transition-all font-medium"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery('')}
              className="absolute right-2.5 p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all"
            >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0 border active:scale-[0.97] ${
              selectedCategory === 'all'
                ...
                : 'bg-slate-100 dark:bg-slate-950/70 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800/80'
            }`}
          >
            <span>☕</span>
            <span>Semua</span>
      ...
      {filteredCatalog.length === 0 && (
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 text-center flex flex-col items-center justify-center gap-2">
          <Search className="w-8 h-8 text-slate-400 dark:text-slate-600 mb-1" />
          <p className="text-sm font-bold text-slate-800 dark:text-slate-300">{t.pos?.noProductsFound || 'Produk tidak ditemukan'}</p>
            ...
            if (suggestion?.suggestedCategory) {
              return (
                <div className="mt-2 flex flex-col items-center gap-1.5 animate-fadeIn">
                  <p className="text-xs text-amber-600 dark:text-amber-300 font-medium">{suggestion.reason}</p>
                  <button
                    type="button"
                    onClick={() => { setSelectedCategory(suggestion.suggestedCategory!); setSearchQuery(''); }}
                    className="px-3.5 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 rounded-xl text-xs font-bold transition-all active:scale-[0.97] flex items-center gap-1.5"
                  >
          ...
          {searchQuery && (
            <button
              ...
              className="mt-2 px-3 py-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-xs font-bold text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white rounded-xl border border-slate-200 dark:border-slate-700 transition-all"
            >
              Reset Pencarian
            </button>
          )}
