  ...
  return (
    <div className="flex flex-col gap-4 w-full text-slate-100">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-sm font-black text-white flex items-center gap-1.5">
            <Sliders className="w-4 h-4 text-amber-400" /> Preferensi Akun Universal Hfe
          </h3>
        ...
        {saveSuccess && (
          <span className="text-[11px] font-bold text-emerald-400 bg-emerald-500/20 px-2.5 py-1 rounded-lg border border-emerald-500/30 flex items-center gap-1 animate-fadeIn">
      ...
      <form onSubmit={handleSave} className="flex flex-col gap-4">
        ...
        <div className="bg-emerald-950/40 border border-emerald-500/40 rounded-2xl p-4 flex items-center justify-between gap-3 shadow-lg">
          <div className="flex items-start gap-2.5 min-w-0">
            ...
            <div>
              <span className="text-xs font-bold text-white block">🌱 Go-Green Paperless Receipts</span>
              <p className="text-[10px] text-emerald-300 font-mono leading-tight mt-0.5">
          ...
          <button
            ...
            className={`w-12 h-6 rounded-full transition-colors relative shrink-0 p-0.5 ${
              preferences.paperlessReceipts ? 'bg-emerald-500' : 'bg-slate-700'
            }`}
          >
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-[11px] text-slate-300 font-medium flex items-center gap-1">
                <Milk className="w-3.5 h-3.5 text-amber-400" /> Pilihan Susu Standar:
              </label>
              <select
                value={preferences.preferredMilk || 'Fresh Milk'}
                onChange={(e) => setPreferences(prev => ({ ...prev, preferredMilk: e.target.value }))}
                className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs font-medium text-white focus:border-amber-500 focus:outline-none"
              >
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[11px] text-slate-300 font-medium flex items-center gap-1">
                <Droplets className="w-3.5 h-3.5 text-amber-400" /> Level Kemanisan Standar:
              </label>
              <select
          ...
          <div className="flex flex-col gap-1.5 pt-1">
            <label className="text-[11px] text-slate-300 font-medium flex items-center gap-1">
              <ShieldAlert className="w-3.5 h-3.5 text-rose-400" /> Peringatan Alergi Makanan (Otomatis Proteksi Semua Dapur):
            </label>
                ...
                return (
                  <button
                    ...
                    className={`text-xs px-2.5 py-1 rounded-xl border transition-all ${
                      isSelected
                        ? 'bg-rose-500/20 border-rose-500/60 text-rose-300 font-bold shadow'
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-4 flex flex-col gap-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-[11px] text-slate-300 font-medium">Plat Nomor Kendaraan (Valet Parkir):</label>
              <input
                type="text"
                ...
                className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs font-mono font-bold text-white focus:border-indigo-500 focus:outline-none uppercase"
              />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-[11px] text-slate-300 font-medium">Alamat Pengiriman Utama:</label>
              <input
                type="text"
                ...
                className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white focus:border-indigo-500 focus:outline-none"
              />
            </div>
          </div>
        </div>
        ...
        <button
          type="submit"
          className="w-full py-3 rounded-2xl bg-amber-500 hover:bg-amber-400 text-slate-950 font-black text-xs flex items-center justify-center gap-2 shadow-xl transition-all transform active:scale-98 cursor-pointer"
        >
