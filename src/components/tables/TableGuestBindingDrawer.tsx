      ...
      <div className="bg-slate-900 border border-slate-800 rounded-t-3xl sm:rounded-3xl max-w-md w-full p-5 flex flex-col gap-4 shadow-2xl relative max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            <div className="w-10 h-10 rounded-xl bg-white text-slate-950 flex items-center justify-center font-bold shadow-md">
              <UserPlus className="w-5 h-5 text-slate-950" />
            </div>
            <div>
              <h3 className="text-sm font-bold text-white flex items-center gap-1.5">
                <span>🛎️ Sambut Tamu & Check-In</span>
                <span className="font-mono text-xs bg-slate-800 text-slate-200 px-2 py-0.5 rounded-lg border border-slate-700">{table.name}</span>
              </h3>
              <p className="text-[11px] text-slate-400">Sambut kedatangan tamu, pilih kontak CRM atau catat tamu walk-in</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white p-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 transition-all"
            title="Tutup"
          >
            <X className="w-4 h-4" />
          </button>
        ...
        <div className="flex bg-slate-950 p-1 rounded-xl border border-slate-800 gap-1">
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${
              bindMode === 'registered' ? 'bg-white text-slate-950 shadow-md font-extrabold' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${
              bindMode === 'walk-in' ? 'bg-white text-slate-950 shadow-md font-extrabold' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-xs font-bold transition-all flex items-center justify-center gap-1.5 ${
              bindMode === 'reservation' ? 'bg-white text-slate-950 shadow-md font-extrabold' : 'text-slate-400 hover:text-white'
            }`}
          >
          ...
          <div className="flex-1 flex flex-col gap-3 overflow-hidden">
            <div className="relative">
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 text-xs rounded-xl pl-9 pr-3 py-2.5 focus:outline-none focus:border-indigo-500 text-slate-100 placeholder:text-slate-500"
              />
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-xs text-white">{guest.name}</span>
                        {guest.isVip && (
                          <span className="text-[9px] font-mono font-bold bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-full border border-amber-500/30 flex items-center gap-1">
                      ...
                      <p className="text-[11px] text-slate-400 mt-1 flex items-center gap-1 font-mono">
                        <Phone className="w-3 h-3 text-slate-500" /> {guest.phone}
                      </p>
                      {guest.pref && (
                        <p className="text-[11px] text-slate-300 mt-1 font-medium italic">Preferensi: {guest.pref}</p>
                      )}
                    </div>
        ...
        {bindMode === 'walk-in' && (
          <div className="flex-1 flex flex-col gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-400 font-semibold">Nama Tamu Walk-In *</label>
              <input
                type="text"
                value={walkInName}
                onChange={(e) => setWalkInName(e.target.value)}
                placeholder="cth: Aldi Pratama"
                className="bg-slate-950 border border-slate-800 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-emerald-500 text-slate-100"
              />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-400 font-semibold">Nomor WhatsApp / HP (Opsional)</label>
              <input
                type="text"
                value={walkInPhone}
                onChange={(e) => setWalkInPhone(e.target.value)}
                placeholder="cth: 081234567890"
                className="bg-slate-950 border border-slate-800 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-emerald-500 text-slate-100 font-mono"
              />
            </div>

            <div className="flex flex-col gap-1">
              <label className="text-xs text-slate-400 font-semibold">Catatan Khusus / Preferensi</label>
              <input
                type="text"
                value={walkInPref}
        ...
        {bindMode === 'reservation' && (
          <div className="flex-1 flex flex-col gap-2 overflow-y-auto pr-1">
            <span className="text-xs font-semibold text-slate-400">Daftar Reservasi Hari Ini:</span>
              ...
              return (
                ...
                >
                  <div>
                    ...
                    <p className="text-[11px] text-slate-400 mt-0.5 font-mono">{res.phone}</p>
                    {res.notes && <p className="text-[10px] text-amber-400/90 mt-1 font-semibold">Catatan: {res.notes}</p>}
                  </div>
        ...
        <div className="pt-3 border-t border-slate-800">
          <button
            ...
            className="w-full bg-indigo-500 hover:bg-indigo-400 disabled:opacity-50 text-white font-bold text-xs py-3 rounded-xl shadow-lg flex items-center justify-center gap-2 transition-all"
          >
