...
export const TableDetailDrawer: React.FC<TableDetailDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-sm flex justify-end transition-all">
      <div className="w-full max-w-lg bg-slate-900 border-l border-slate-800 h-full flex flex-col justify-between p-4 sm:p-6 overflow-y-auto shadow-2xl animate-in slide-in-from-right duration-200">
        ...
        <div className="flex flex-col gap-3 border-b border-slate-800 pb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2">
                🗺️ {table.name} <span className="text-xs font-mono font-normal text-slate-400">({table.status.toUpperCase()})</span>
              </h2>
            </div>
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 text-slate-400 hover:text-white bg-slate-800 rounded-xl transition-all"
            >
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 bg-slate-950 p-3 rounded-2xl border border-slate-800 text-xs">
            <div className="flex items-center gap-2 text-slate-300">
              ...
              <div>
                <p className="text-[10px] text-slate-400">Waktu Menunggu</p>
                <p className="font-mono font-bold text-amber-400">18 Menit</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 text-slate-300">
              ...
              <div>
                <p className="text-[10px] text-slate-400">Jumlah Tamu</p>
                <p className="font-mono font-bold text-slate-200">3 Orang (3 Seats)</p>
              </div>
            </div>
          </div>

          {/* STATUS JOIN MEJA */}
          <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-xl p-2.5 flex items-center justify-between text-xs">
            ...
            <button
              ...
              className="text-[11px] font-bold text-rose-400 hover:text-rose-300 bg-rose-500/20 px-2 py-1 rounded-lg border border-rose-500/40 flex items-center gap-1"
            >
        ...
        <div className="flex-1 py-4 flex flex-col gap-4 overflow-y-auto">
          <h3 className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
            <Utensils className="w-3.5 h-3.5 text-amber-400" /> Rincian Pesanan per-Seat / Tamu:
          </h3>

          <div className="flex flex-col gap-3">
            {mockSeats.map((seat) => (
              <div key={seat.seatNo} className="bg-slate-950 border border-slate-800 rounded-2xl p-3 flex flex-col gap-2">
                <div className="flex items-center justify-between border-b border-slate-800/80 pb-2">
                  <div className="flex items-center gap-2">
                    <UserCheck className="w-3.5 h-3.5 text-indigo-400" />
                    <span className="text-xs font-bold text-slate-200">Seat {seat.seatNo} — {seat.guestName}</span>
                  </div>
                  ...
                  <button
                    ...
                    className="text-[10px] font-bold text-amber-400 hover:text-amber-300 bg-amber-500/10 hover:bg-amber-500/20 px-2 py-0.5 rounded-lg border border-amber-500/30 flex items-center gap-1"
                  ...
                  {seat.items.map((item) => (
                    <div key={item.id} className="flex items-center justify-between text-xs">
                      <div className="flex items-center gap-2 min-w-0">
                        <span className={`w-1.5 h-1.5 rounded-full ${item.status === 'served' ? 'bg-emerald-400' : 'bg-amber-400 animate-pulse'}`} />
                        <span className="text-slate-300 truncate">{item.name} x{item.qty}</span>
                      </div>
        ...
        <div className="border-t border-slate-800 pt-4 flex flex-col gap-3">
          <div className="bg-slate-950 p-3 rounded-2xl border border-slate-800 flex flex-col gap-1.5 text-xs">
            ...
            <div className="flex justify-between text-slate-400">
              ...
              <span className="font-mono text-indigo-400">Rp {pb1Tax.toLocaleString('id-ID')}</span>
            </div>
            <div className="flex justify-between text-sm font-extrabold text-white border-t border-slate-800/80 pt-1.5 mt-0.5">
              <span>Sisa Tagihan Meja:</span>
              <span className="font-mono text-emerald-400">Rp {remainingPayable.toLocaleString('id-ID')}</span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs rounded-xl flex items-center justify-center gap-1.5 transition-all"
            >
              <Plus className="w-4 h-4 text-indigo-400" /> + Tambah Menu
            </button>
            <button
              type="button"
