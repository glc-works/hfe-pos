...
PILLAR_NAME_MAP = {
    ...
    "react_aria": 2, "aria": 2, "primitives": 2, "p2": 2,
    "spatial_isolation": 3, "spatial": 3, "collision": 3, "ui": 3, "viewport": 3, "p3": 3,
...
def run_cadence(
    ...
    """Run cadence feedback loop: SMALL (fast static & in-memory), MEDIUM (typecheck, tests), or LARGE (build, e2e)."""
    if root_dir is None:
        root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    ...
    # Fast in-memory pillars for SMALL cadence
    ...
    # Full 9 pillars for MEDIUM / LARGE cadence
